...
test-unit:
	...
	do \
		if [ -d "src/$$SERVICE/tests" ]; then \
			(cd src/$$SERVICE && uv run pytest -v -p no:warnings --cov=. --cov-branch --cov-report=term-missing --cov-report=xml) || exit 1; \
		else \
			echo "src/$$SERVICE has no tests/ directory, skipping"; \
		fi; \
	done
