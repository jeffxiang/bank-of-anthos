...
class BalanceReaderControllerTest {
    ...
    private static final String MALFORMED_ACCOUNT_NUM = "12341545\uD83D\uDC3B";
    ...
    void getBalance_WhenCacheThrowsUncheckedExecutionException_Returns500() throws Exception {
        ...
        when(cache.get(AUTHED_ACCOUNT_NUM)).thenThrow(
            new UncheckedExecutionException(new RuntimeException("cache backend down")));
    ...
    @DisplayName("Given an Authorization header without the Bearer prefix, "
            + "verify the raw token and return the balance")
    void getBalance_WhenAuthorizationHeaderHasNoBearerPrefix_VerifiesRawToken() throws Exception {
        // Given
        when(claim.asString()).thenReturn(AUTHED_ACCOUNT_NUM);
        when(cache.get(AUTHED_ACCOUNT_NUM)).thenReturn(BALANCE);
    ...
    @Test
    @DisplayName("Given a valid token with no account claim, return 401 without hitting the cache")
    void getBalance_WhenTokenHasNoAccountClaim_Returns401() throws Exception {
        // Given
    ...
    @Test
    @DisplayName("Given a malformed unicode account id, return 401 without hitting the cache")
    void getBalance_WhenAccountIdIsMalformedUnicode_Returns401() throws Exception {
        // Given
        when(claim.asString()).thenReturn(AUTHED_ACCOUNT_NUM);

        // When
        final ResponseEntity actualResult = balanceReaderController.getBalance(
            BEARER_TOKEN, MALFORMED_ACCOUNT_NUM);
